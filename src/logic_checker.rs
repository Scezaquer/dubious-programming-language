            ...
            match atom {
                ...
                Atom::MemberAccess(id, member) => {
					...
					let t = context
                            .variables
                            .get(id)
                            .expect(format!("Undefined variable {}", id).as_str());
					if let Type::Struct(id) = t {
						if let Some((_, unordered_list)) = context.structs.get(id) {
                    		if let Atom::Variable(member) = member.as_ref() {
                            	if let Some(t) = unordered_list.get(member) {
                                    return t.clone();
                                } else {
                                    panic!("Struct '{}' does not have member '{}'", id, member);
                                }
							} else if let Atom::MemberAccess(id, sub_member) = member.as_ref() {
								if let Some(t) = unordered_list.get(id) {
									return type_expression(&Expression::Atom(sub_member.as_ref().clone()), context);
								} else {
									panic!("Struct '{}' does not have member '{}'", id, member);
								}
							} else {
...
fn type_statement(statement: &Statement, context: &Context) -> Type {
    match statement {
        ...
        Statement::Break => return Type::Void,
        Statement::Continue => return Type::Void,
        Statement::Loop(body) => {
            return type_statement(body, context);
        }
