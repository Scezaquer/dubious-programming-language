...
pub fn preprocessor(
    ...
    let preprocessor_re = Regex::new(
        r"^\#(include|define|undef|ifdef|ifndef|else|endif|error|print|namespace|spacename)",
    )
    .unwrap();
    ...
    while pos < file.len() {
        let rest = &file[pos..];
        if rest.starts_with("#")
            && let Some(caps) = preprocessor_re.captures(&file[pos..])
        {
            ...
            match directive {
                "include" => {
                    ...
                    let (actual_path, is_directory) = if full_path.is_dir() {
                        ...
                        if include_file.exists() {
                            ...
                            panic!(
                                "{} Line {}: Directory does not contain include.dpl: {}",
                                filename,
                                line,
                                full_path.display()
                            );
                    ...
                    let included_file = preprocessor(
                        &included_file,
                    ...
                    if let Some(caps) = identifier_re.captures(rest) {
...

                        {
                            ...
                            if defined_expressions.contains_key(identifier) {
                                println!(
                                    "{} Line {} Warning: #define identifier '{}' is already defined",
                ...
                "undef" => {
...

                    {
                        ...
                        if !defined_expressions.contains_key(identifier) {
                            println!(
                                "{} Line {} Warning: #undef identifier '{}' is not defined",
                                filename, line, identifier
                            );
                        }
                ...
                "namespace" => {
...

                    {
                        ...
                        if defined_expressions.contains_key(namespace) {
                            println!(
                                "{} Line {} Warning: #namespace identifier '{}' is already defined",
                                filename, line, namespace
                            );
                        }
                    }
                ...
                "spacename" => {
                    ...
                    if namespace_counter < 0 {
                        panic!(
                            "{} Line {}: #spacename directive without matching #namespace",
                            filename, line
                        );
                    }
            ...
            && let Some(caps) = comments_re.captures(rest)
        {
            pos += caps.get(0).unwrap().end();
            line += 1;
            processed_file.push_str(&"\n");
        } else if rest.starts_with("/*")
            && let Some(caps) = multiline_comments_re.captures(rest)
        {
            pos += caps.get(0).unwrap().end();
